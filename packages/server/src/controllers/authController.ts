...
import { generateOtp, sendVerificationEmail } from "../services/mailjetService";
import crypto from "crypto";
import { prisma } from "../utils/prisma";
import { OTP_EXPIRY_MINUTES } from "../utils/env";
...
export class AuthController {
  ...
  static async verifyOtp(req: AuthRequest, res: Response) {
    try {
      ...
      const temp = await prisma.tempRegistration.findFirst({
        where: { email: emailLower, otpCode: otp },
      });
  ...
  static async resendOtp(req: AuthRequest, res: Response) {
    try {
      ...
      const tempRow = await prisma.tempRegistration.findUnique({
        where: { email: emailLower },
      });
...
export class GoogleAuthController {
  static async redirectToGoogle(req: AuthRequest, res: Response) {
    try {
      ...
      console.log(`[GoogleAuth] Initiating redirect with URI: ${redirectUri}`);
