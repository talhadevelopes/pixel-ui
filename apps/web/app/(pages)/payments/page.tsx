        ...
        {!isBusy &&
          planCards.map((plan: PlanCard) => {
            ...
            return (
              <Card
                key={plan.tier}
                className={plan.isCurrent ? "border-primary" : undefined}
              >
                <CardHeader>
                  <div className="flex items-center justify-between">
                ...
                <CardFooter>
                  {plan.isPaidTier ? (
                    <Button
                      ...
                      className="w-full"
                      onClick={() =>
                        handleSubscribe(plan.tier as "pro" | "premium")
                      }
                    >
      ...
      <div className="flex flex-col gap-2 rounded-lg border bg-card p-6">
        ...
        <div className="grid grid-cols-1 gap-4 text-sm text-muted-foreground md:grid-cols-3">
          ...
          <div>
            ...
            <p>
              {statusData?.credits ?? 0} / {statusData?.dailyCreditsLimit ?? 3}
            </p>
          </div>
          <div>
