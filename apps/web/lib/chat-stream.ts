...
export async function parseChatCompletionStream(response: Response, options?: ChatStreamOptions) {
    ...
    let codeBuffer = "";
    let isCode = false;
    const htmlStartRegex = /<\s*(section|div|header|main|footer|nav|ul|ol|article|aside|form|img|h1|h2|h3|h4|h5|h6|button|a|span|p|table|figure|canvas|svg)[\s>]/i;
...
export function stripCodeFences(code: string) {
    return code.replace(/```\s*html/gi, "").replace(/```/g, "").trim();
}
